...
class ProductTemplate(models.Model):
    ...
    no_create_variants = fields.Selection([('yes', u"不自动创建产品变体"),
                                           ('no', "自动创建产品变体"),
                                           ('empty', '使用分类上的设置')],
                                          string=u'变体创建', required=True, default='yes',
    ...
    def onchange_category(self):
        '''更改分类时，自动配置模板的属性设置'''
        print('==onchange catogory 1==', self.id, self.env.context)
        line_obj = self.env['product.attribute.line']
        attribute_lines = line_obj.browse([])
        for x in self.categ_id.categ_attr_line_ids:
            line = line_obj.new({
                'product_tmpl_id': self.id,
                'attribute_id': x.attribute_id.id,
                'required': x.required,
                'value_ids': [(6, 0, [i.id for i in x.value_ids])],
            })
