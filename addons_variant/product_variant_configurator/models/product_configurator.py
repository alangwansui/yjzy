...
class ProductConfigurator(models.AbstractModel):
    ...
    def _set_product_tmpl_attributes(self):
        if self.product_tmpl_id:
            ...
            for attribute_line in self.product_tmpl_id.attribute_line_ids.sorted(key='attribute_id'):
                #print('>>>', attribute_line, attribute_line.attribute_id, attribute_line.attribute_id.name,  attribute_line.attribute_id.sequence)
    ...
    def _onchange_product_id_configurator(self):
        ...
        if self.product_id:
            ...
            pass
            # 防止设置设置过程中，遇到重复的属相了，但还没保存的情况，就自动onchange属性，造成用户设置进程中断
            #self._set_product_tmpl_attributes()
        else:
            self._empty_attributes()
