...
class ProductProduct(models.Model):
    ...
    def create(self, vals):
        print('#####create1', vals.get('attribute_value_ids'))
        ...
        ctx.update({'product_name': vals.get('name', '')})
        if ('product_tmpl_id' in vals) and ('default_name' in ctx):
            ctx.pop('default_name')


        obj = self.with_context(ctx)
        print('#####create3', ctx,  vals)
