...
class HrExpense(models.Model):
    ...
    date = fields.Date(readonly=True, states={'draft': [('readonly', False)], 'refused': [('readonly', False)]}, required=True,  string="Expense Date") #default=fields.Date.context_today,akiny修改原生
    ...
    product_id = fields.Many2one('product.product', string='Product', readonly=False, states={'draft': [('readonly', False)], 'refused': [('readonly', False)]}, domain=[('can_be_expensed', '=', True)], required=True)
    ...
    def _prepare_move_line(self, line):
        ...
        '''
        partner_id = line.get('partner_id') or self.employee_id.address_home_id.commercial_partner_id.id
        return {
...
class HrExpenseSheet(models.Model):
    ...
    def action_get_attachment_view(self):
        ...
        self.ensure_one()
        res = self.env['ir.actions.act_window'].for_xml_id('base', 'action_attachment')
        res['domain'] = [('res_model', '=', 'hr.expense.sheet'), ('res_id', 'in', self.ids)]
