...
class Message(models.Model):
    ...
    def _get_default_from(self):
        if self.env.user.email:
            return '%s <%s>' % (self.env.user.name, self.env.user.email)
    ...
    def create(self, values):
        ...
        #print('*******1', message.partner_cc_ids, values)


        if tracking_values_cmd:
            message.sudo().write({'tracking_value_ids': tracking_values_cmd})

        #print('*******1.1', message, self.env['mail.mail'].search([('mail_message_id', '=', message.id)], ))

        message._invalidate_documents()

        #print('*******1.2', message, self.env['mail.mail'].search([('mail_message_id', '=', message.id)], ))
    ...
    def _notify(self, force_send=False, send_after_commit=True, user_signature=True):
        ...
        if self.force_notify_email:
            ...
            if not todo_partners:
                todo_partners = self.env.user.partner_id

            todo_partners._notify(self, force_send=force_send, send_after_commit=send_after_commit, user_signature=user_signature)
        ...
        # # <jon if not partner, user current user to send email>
        # elif self._context.get('send_by_uid'):
        #     partener = self.env.user.partner_id
        #     partener.sudo()._notify(self, force_send=force_send, send_after_commit=send_after_commit, user_signature=user_signature)
