   ...
   Only allowed people can contact us at this address.</p>
    ...
    def message_route(self, message, message_dict, model=None, thread_id=None, custom_values=None, fetch_server=None):
        ...
        rcpt_tos = ','.join([
            ...
            tools.decode_message_header(message, 'Received'),
        ...
        #         is_a_reply = False
        # print('==message_route==4',is_a_reply)
        ...
        #         allow_private=reply_private, drop_alias=False)
        #     if route:
        #         _logger.info(
        ...
        print('==message_route==6', fallback_model)
        if fallback_model:
            # no route found for a matching reference (or reply), so parent is invalid
            message_dict.pop('parent_id', None)
            route = self.message_route_verify(
    ...
    def message_process(self, model, message, custom_values=None,
        ...
        if msg.get('message_id'):   # should always be True as message_parse generate one if missing
            #pass
            #<jon> existing_msg_ids = self.env['mail.message'].search([('message_id', '=', msg.get('message_id'))])
            #增加邮件归档的搜索，同时允许自己发出的邮件自己收取

            existing_msg_ids = self.env['mail.message'].with_context(active_test=False).search([('process_type', '=', 'in'), ('message_id', '=', msg.get('message_id'))])
            if existing_msg_ids:
    ...
    def message_parse(self, message, save_original=False):
        ...
        #<jon> parse partner or 'To' and 'Cc '
        partner_ids = self._message_find_partners(message, ['To'])
        partner_cc_ids = self._message_find_partners(message, ['Cc'])

        msg_dict['partner_ids'] = [(4, partner_id) for partner_id in partner_ids]
        msg_dict['partner_cc_ids'] = [(4, partner_id) for partner_id in partner_cc_ids]

        #print('===message_parser====', msg_dict)
        ...
        if message.get('Received'):
            bcc_mail = parseaddr(message.get('Received'))[1]
            if bcc_mail:
                msg_dict.update({'bcc': bcc_mail, 'email_bcc': bcc_mail})
