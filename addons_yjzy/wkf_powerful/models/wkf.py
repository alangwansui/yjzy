...
class wkf_base(models.Model):
    ...
    model_view_ids = fields.Many2many('ir.ui.view', 'ref_view_wkf', 'wid', 'vid', u'模型视图')
    ...
    view_ids = fields.Many2many('ir.ui.view', 'ref2_view_wkf', 'wid', 'vid', u'WKF视图')
    ...
    def make_btm_contain(self):
        ...
        btn_contain.append(XML(btn_show_log_template % {'btn_str': u'显示日志', 'btn_grp': 'base.group_user'} ))
        btn_contain.append(XML(btn_wkf_reset_template % {'btn_str': u'重置工作流', 'btn_grp': 'base.group_system', 'btn_ctx': self.id, 'no_reset_states': self.no_reset_states }  ))
        return btn_contain
    ...
    def make_view(self):
        ...
        new_views = view_obj.browse([])
        for ov in self.model_view_ids:

            have_header = '<header>' in ov.arch
            ...
            name = 'WKF.inherit.%s' % ov.name

            v = view_obj.search([('name', '=', name),('inherit_id', '=', ov.id)], limit=1)
            if v:
                v.arch = tostring(arch)
            else:
                view_data = {
                    'name':  name,
                    'type': 'form',
                ...
                v = view_obj.create(view_data)

            new_views |= v

        self.view_ids = new_views
        return True
...
class wkf_node(models.Model):
    ...
    action2 = fields.Char('动作2',size=64, help='When arrived this node, you can set to trigger a object function to do something, example confirm the order')
    arg2 = fields.Text('参数2',size=64, help='the object function args')

    action3 = fields.Char('动作3',size=64, help='When arrived this node, you can set to trigger a object function to do something, example confirm the order')
    arg3 = fields.Text('参数3',size=64, help='the object function args')
    ...
    event_py_code = fields.Text(u'Py Code')
    ...
    def make_event(self, name, record):
        #print('>>>>>>>>', name, self.env.context)


        #print('===', self.event_py_code)
        partner_ids = [u.partner_id.id for u in self.event_users]

        #print('======', partner_ids)
        if self.event_py_code:
            partner_ids += eval(self.event_py_code)
        ...
        message = self.env['mail.message'].create(data)
...
class wkf_trans(models.Model):
    ...
    user_ids = fields.Many2many('res.users', 'user_trans_ref', 'tid', 'uid',  'Users', help="The Users who can process this transfer")
