...
class PopupReminder(models.Model):
    ...
    def get_total_data(self):
        ...
        for data in reminder_ids:
            ...
            model_count.update({ustr(data.name): count})
    ...
    def get_record_header(self):
        ...
        for data in reminder_ids:
            field_res = {}
            field_label = []
            res_model.update({ustr(data.name): data.model_id.model})
            for display_data in data.popup_field_ids:
                field_res.update({ustr(display_data.name): ustr(
                    display_data.field_description)})
            field_label.append(field_res)
            res.update({ustr(data.name): field_label})
