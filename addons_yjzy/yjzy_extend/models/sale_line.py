...
class sale_order_line(models.Model):
    ...
    price_total2 = fields.Monetary(u'销售金额',
                                   currency_field='company_currency_id', )  # 'sale_amount': sol.price_total,compute=compute_info
    ...
    product_pi_description = fields.Text('PI Description', related='product_id.pi_description')
    ...
    purchase_payment_term = fields.Many2one('account.payment.term', u'供应商付款条款',
                                            related='pol_id.order_id.payment_term_id')
    ...
    def compute_tb_line_count(self):
        for one in self:
            one.tb_line_count = len(one.tbl_ids.filtered(
    ...
    @api.depends('product_id', 'supplier_id', 'product_id.variant_seller_ids.product_name',
    ...
    # m2m不可以随便加入depends
    @api.depends('tbl_ids', 'tbl_ids.plan_qty', 'tbl_ids.qty2stage_new', 'product_uom_qty', 'order_id.state',
                 'price_unit')
    ...
    def product_id_change(self):
        ...
        self.supplier_id = self.product_id.variant_seller_ids and self.product_id.variant_seller_ids[
            0].name.is_ok == True and self.product_id.variant_seller_ids[0].name.id or None,
        return res
