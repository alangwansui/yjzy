...
class Product_Product(models.Model):
    ...
    so_line_ids = fields.One2many('sale.order.line', 'product_id', u'销售明细', domain=[
    ...
    seq = fields.Char(u'规格流水号', readonly=False, copy=False,
                      default=lambda self: self.env['ir.sequence'].next_by_code('product.product'))
    ...
    packaging_ids = fields.One2many(
        'product.packaging', 'product_id', 'Product Packages', default=lambda self: self._default_packaging(),
        copy=True)
    ...
    state = fields.Selection([('draft', u'草稿'), ('submit', u'待业务责任人审批'), ('first', '待产品合规审批'), ('done', u'完成')], u'状态',
                             default='draft')
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        products = self.search(['|', ('customer_ref', operator, name), ('customer_ref2', operator, name)] + args,
                               limit=limit)
    ...
    def open_wizard_attribute_configurator(self):
        ...
        for remaind_attr in (
                self.attribute_line_ids.mapped('attribute_id') - self.attribute_value_ids.mapped('attribute_id')):
            line_obj.create({
                'wizard_id': wizard.id,
    ...
    def get_package_info(self, qty):
        ...
        gross_weight = (
                    max_qty * max_package.weight4product)  ###中小包装不参与计算 + ( mid_qty * mid_package.weight4product) + (min_qty * min_package.weight4product)
