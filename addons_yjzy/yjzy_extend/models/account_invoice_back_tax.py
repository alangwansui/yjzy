...
class account_invoice(models.Model):
    ...
    real_invoice_auto_state_1 = fields.Selection(
        [('10', '报关数据待锁定'), ('20', '报关数据已锁定-应收发票待锁定'), ('30', '应收发票锁定-发票未收齐'), ('40', '发票已收齐-未开销项'),
         ('50', '已开销项-未申报退税'), ('60', '已申报退税未收退税'), ('70', '已收退税')], index=True,
    ...
    back_tax_declaration_invoice_ids = fields.One2many('back.tax.declaration', 'back_tax_all_in_one_invoice_id',
                                                       u'退税申报')
    ...
    def compute_back_tax_state_group(self):
        for one in self:
            state = one.state
            back_tax_declaration_state = one.back_tax_declaration_state
            if state == 'draft' :
                back_tax_state_group = '10'
            elif state == 'open' and back_tax_declaration_state == '10':
                ...
                back_tax_state_group = '50'
            one.back_tax_state_group = back_tax_state_group