...
class AccountReconcileStage(models.Model):
    ...
    state = fields.Selection(Account_reconcile_Selection, 'State',
                             default=Account_reconcile_Selection[0][0])  # track_visibility='onchange',
...
class account_reconcile_order(models.Model):
    ...
    def compute_by_lines(self):
        for one in self:
            ...
            one.amount_payment_org = payment_currency.compute(
                sum([x.amount_payment_org for x in lines]) + sum([x.amount_payment_org for x in lines_no]),
                one.invoice_currency_id)
            one.amount_payment = sum([x.amount_payment for x in lines])
            one.amount_total_org = sum([x.amount_total_org for x in lines]) + sum(
                [x.amount_payment_org for x in lines_no])
    ...
    def default_payment_account(self):
        ...
        bank_account = account_obj.search([('code', '=', '112301'), ('company_id', '=', self.env.user.company_id.id)],
                                          limit=1)
    ...
    def default_bank_account(self):
        account_obj = self.env['account.account']
        bank_account = account_obj.search([('code', '=', '5603'), ('company_id', '=', self.env.user.company_id.id)],
                                          limit=1)
        return bank_account and bank_account.id

    def default_diff_account(self):
        account_obj = self.env['account.account']
        diff_account = account_obj.search([('code', '=', '5601'), ('company_id', '=', self.env.user.company_id.id)],
                                          limit=1)
        return diff_account and diff_account.id

    def default_exchange_account(self):
        account_obj = self.env['account.account']
        diff_account = account_obj.search([('code', '=', '5712'), ('company_id', '=', self.env.user.company_id.id)],
                                          limit=1)
        return diff_account and diff_account.id
        ...
        for one in self:
            ...
            if one.sfk_type == 'yfhxd':
                ...
                supplier_advance_payment_ids = self.env['account.payment'].search(
                    [('company_id', '=', one.company_id.id), ('partner_id', '=', one.partner_id.id),
                     ('sfk_type', '=', 'yfsqd'), ('po_id', 'in', po),
    ...
    @api.depends('line_ids', 'line_ids.amount_payment_org', 'payment_currency_id', 'line_no_ids',
                 'line_no_ids.amount_payment_org')
    def compute_amount_payment_org_new(self):
        for one in self:
            ...
            lines = one.line_ids
            lines_no = one.line_no_ids
            amount_payment_org = sum([x.amount_payment_org for x in lines]) + sum(
                [x.amount_payment_org for x in lines_no])
    ...
    def compute_amount_diff_org_new(self):
        for one in self:
            ...
            diff_currency = one.payment_currency_id.with_context(date=date)
            lines = one.line_ids
            one.amount_diff_org_new = diff_currency.compute(sum([x.amount_diff_org for x in lines]),
                                                            one.invoice_currency_id)

    @api.depends('line_ids', 'line_ids.amount_total_org_new', 'line_ids.amount_advance_org',
                 'line_ids.amount_payment_org', 'line_no_ids',
    ...
    def compute_amount_total_org_new(self):
        for one in self:
            ...
            amount_total_org_new_percent = invoice_amount_total != 0 and (
                    amount_line_no + amount_line) / invoice_amount_total or 0
    ...
    def compute_advance_reconcile_line_draft_all_count(self):
        for one in self:
            one.advance_reconcile_line_draft_all_count = sum(
                x.advance_reconcile_order_draft_ids_count for x in one.supplier_advance_payment_ids)
    ...
    def compute_supplier_advance_payment_ids_amount_advance_org(self):
        for one in self:
            supplier_advance_payment_ids_amount_advance_org = sum(
                x.advance_reconcile_order_draft_amount_advance for x in one.supplier_advance_payment_ids)
    ...
    @api.depends('yjzy_advance_payment_id', 'yjzy_advance_payment_id.amount_advance_org_all',
                 'yjzy_advance_payment_id.advice_amount_advance_org_all')
    ...
    @api.depends('partner_id', 'partner_id.supplier_amount_invoice_approval',
                 'partner_id.supplier_amount_invoice_approve',
    ...
    def compute_supplier_amount_invoice_approve_approval(self):
        for one in self:
            supplier_invoice_ids = one.partner_id.supplier_invoice_ids.filtered(
            ...
            supplier_advance_amount_hxd_line_approval = sum(
                x.advance_amount_reconcile_order_line_approval for x in supplier_advance_payment_ids)
            supplier_amount_residual_advance_payment = sum(
                x.advance_balance_total for x in supplier_advance_payment_ids)
    ...
    @api.depends('account_payment_state_ids', 'account_payment_state_ids.amount_advance_balance_d',
                 'account_payment_state_ids.amount_reconcile',
    ...
    def compute_account_payment_state_ids(self):
        for one in self:
            ...
            one.account_payment_state_ids_amount_2 = sum(
    ...
    def compute_supplier_amount_residual_advance_payment(self):
        for one in self:
            ...
            one.supplier_amount_residual_advance_payment = sum(
                x.advance_balance_total for x in supplier_advance_payment_ids)
    ...
    def compute_invoice_id(self):
        for one in self:
            if one.invoice_ids:
                if len(one.invoice_ids.filtered(
                        lambda x: x.yjzy_type == 'back_tax' or x.yjzy_type_1 == 'back_tax')) > 1:
    ...
    invoice_reconcile_order_line_no_ids = fields.One2many('account.reconcile.order.line.no',
                                                          related='invoice_id.reconcile_order_line_no_ids')
    invoice_reconcile_order_line_no_ids_count = fields.Integer(u'no数量',
                                                               related='invoice_id.reconcile_order_line_no_ids_count')
    ...
    amount_payment_can_approve_all_after = fields.Monetary('所有账单本次申请后可申请支付金额合计', currency_field='invoice_currency_id',
                                                           compute=compute_amount_payment_can_approve_all_after)
    amount_payment_can_approve_all_this_time = fields.Monetary('所有账单本次申请前可申请支付金额合计',
    ...
    ysrld_amount_advance_org_all = fields.Float('预收单的本所有被认领金额', compute=compute_ysrld_amount_advance_org_all,
                                                store=True)
    ysrld_advice_amount_advance_org_all = fields.Float('预收认领单的所有被认领的原则分配金额',
                                                       compute=compute_ysrld_amount_advance_org_all, store=True)
    duoyu_this_time_advice_advance_org = fields.Float('多余的预收付这次应该加上的认领金额', compute=compute_ysrld_amount_advance_org_all,
                                                      store=True)

    yjzy_reconcile_order_id_new = fields.Many2one('account.reconcile.order',
                                                  '关联的核销单')  # 1126先创建预付，再创建应付，两边同时相互记录,ondelete='cascade'

    advice_amount_advance_org_total = fields.Monetary(u'建议预收金额', currency_field='yjzy_advance_currency_id',
                                                      compute=compute_advice_amount_advance_org_total, store=True)
    least_advice_amount_advance_org = fields.Monetary(u'最低预收预付金额', currency_field='yjzy_advance_currency_id',
                                                      compute=compute_advice_amount_advance_org_total, store=True)
    # 0104
    yjzy_reconcile_order_id = fields.Many2one('account.reconcile.order',
                                              '关联的核销单')  # 从付款申请认领的时候，创建预付申请，让两者之间产生关联,ondelete='cascade'
    ...
    yjzy_reconcile_order_approval_ids = fields.One2many('account.reconcile.order', 'yjzy_reconcile_order_id',
                                                        u'相关待审批预付-应付认领',
                                                        domain=[('state_1', '=', 'manager_approval')],
                                                        )  # 从自身创建的所有的预付-应付认领单。

    renling_type = fields.Selection([('yshxd', '应收认领'),
                                     ('back_tax', '退税认领'), ('new_back_tax', '退税认领'), ('other_payment', '其他认领'),
                                     ('purchase_add_invoice', '增加采购应收认领')], u'认领属性')  # 没有是指作用
    ...
    supplier_advance_payment_ids_amount_advance_org = fields.Float('待审批预付认领金额',
                                                                   compute=compute_supplier_advance_payment_ids_amount_advance_org)
    ...
    supplier_amount_invoice_residual = fields.Float(u'供应商应付余额',
                                                    compute=compute_supplier_amount_invoice_approve_approval)
    supplier_amount_invoice_residual2 = fields.Float(u'供应商应付余额2',
                                                     compute=compute_supplier_amount_invoice_approve_approval,
                                                     )

    supplier_advance_amount_hxd_line_approval = fields.Float(u'供应商预付审批中预付认领',
                                                             compute=compute_supplier_amount_invoice_approve_approval)
    supplier_advance_amount_hxd_line_approval2 = fields.Float(u'供应商预付余额2',
                                                              compute=compute_supplier_amount_invoice_approve_approval)
    ...
    state_1 = fields.Selection(Account_reconcile_Selection, u'审批流程', default='draft', index=True,
    ...
    advance_reconcile_line_draft_all_count = fields.Integer('未完成审批的预付认领单数量',
                                                            compute=compute_advance_reconcile_line_draft_all_count)

    expense_sheet_id = fields.Many2one('hr.expense.sheet', u'费用报告')
    supplier_advance_payment_ids = fields.Many2many('account.payment', u'相关预收付',
                                                    compute=compute_supplier_advance_payment_ids)  # 相关的预收和预付
    ...
    yjzy_advance_payment_balance = fields.Monetary('预付款单余额', currency_field='yjzy_advance_currency_id',
                                                   related='yjzy_advance_payment_id.advance_balance_total')
    yjzy_advance_payment_amount = fields.Monetary('预付款单原始金额', currency_field='yjzy_advance_currency_id',
                                                  related='yjzy_advance_payment_id.amount')
    ...
    payment_type = fields.Selection(
        [('outbound', u'付款'), ('inbound', u'收款'), ('claim_in', u'收款认领'), ('claim_out', u'付款认领')], string=u'收/付款',
        required=True)
    ...
    amount_purchase_advance = fields.Monetary('预付金额:本币', currency_field='currency_id',
                                              related='partner_id.amount_purchase_advance')
    ...
    supplier_amount_residual_advance_payment = fields.Float('预付余额',
                                                            compute=compute_supplier_amount_residual_advance_payment)

    supplier_no_received_amount = fields.Float('总未发货金额', compute=compute_supplier_no_received_amount)
    ...
    amount_invoice_residual_org = fields.Monetary(u'发票余额', currency_field='invoice_currency_id',
    ...
    amount_advance_org_new = fields.Monetary(u'使用预收', currency_field='invoice_currency_id', store=True,
                                             compute=compute_amount_advance_org_new)  # 0909新
    amount_payment_org = fields.Monetary(u'收款金额', currency_field='invoice_currency_id', compute=compute_by_lines)
    amount_payment_org_new = fields.Monetary(u'收款金额', currency_field='invoice_currency_id',
                                             compute=compute_amount_payment_org_new, store=True)  # 0909新
    amount_bank_org = fields.Monetary(u'银行扣款', currency_field='invoice_currency_id', compute=compute_by_lines)
    amount_bank_org_new = fields.Monetary(u'银行扣款', currency_field='invoice_currency_id',
                                          compute=compute_amount_bank_org_new, store=True)  # 0909新
    amount_diff_org = fields.Monetary(u'销售费用', currency_field='invoice_currency_id', compute=compute_by_lines)
    amount_diff_org_new = fields.Monetary(u'销售费用', currency_field='invoice_currency_id',
                                          compute=compute_amount_diff_org_new, store=True)  # 0909新
    # amount_exchange_org = fields.Monetary(u'汇兑差异', currency_field='invoice_currency_id', compute=compute_by_lines)
    amount_total_org = fields.Monetary(u'收款合计', currency_field='invoice_currency_id', compute=compute_by_lines,
                                       store=False)
    ...
    feiyong_product_id = fields.Many2one('product.product', u'费用产品', domain=[('type', '=', 'service')],
                                         default=_default_feiyong_product)
    fygb_id = fields.Many2one('hr.expense.sheet', u'费用报告')

    back_tax_product_id = fields.Many2one('product.product', u'退税产品', domain=[('type', '=', 'service')],
                                          default=_default_back_tax_product)
    ...
    def open_po_no_received(self):
        tree_view = self.env.ref('yjzy_extend.new_purchase_order_with_sale_order_tree').id
        form_view = self.env.ref('yjzy_extend.new_purchase_order_from').id
        return {'name': '未发货完成的采购合同',
                'view_type': 'form',
                'view_mode': 'tree, form',
        ...
        for one in self.line_ids.filtered(lambda x: x.amount_advance_org != 0):
            for line in self.move_ids:
                for x in line.line_ids:
                    ...
                    if (x.account_id.code in ['2203', '1123'] and (
                            x.amount_currency == one.amount_advance_org or x.amount_currency == -one.amount_advance_org)) and one.yjzy_payment_id and x.new_advance_payment_id != one.yjzy_payment_id:
                        x.new_advance_payment_id = one.yjzy_payment_id
    ...
    def name_get(self):
        ...
        for one in self:
            ...
            elif ctx.get('default_sfk_type',
                         '') == 'yfhxd' and one.hxd_type_new == '40' and one.operation_wizard != '03':
                ...
                if operation_wizard == '20':  # 预收认领
                    ...
                    for one in lines:
                        if one.so_id:
                            ...
                            if one.advance_residual >= 0 and one.amount_advance_org > one.advance_residual:
    ...
    def action_confirm_ysrld_stage(self):
        ...
        if self.invoice_attribute_all_in_one != '640':
            for one in self.line_no_ids:
                ...
                print('amount_payment_org_akiny',
                      one.payment_currency_id.compute(one.amount_payment_org, one.invoice_currency_id))
                if one.amount_payment_can_approve_all >= 0 and  float_compare(one.amount_payment_can_approve_all,one.payment_currency_id.compute(
                        one.amount_payment_org, one.invoice_currency_id),precision_digits=2) == 0:

                        # one.amount_payment_can_approve_all < one.payment_currency_id.compute(
                        # one.amount_payment_org, one.invoice_currency_id):
    ...
    def create_customer_invoice(self):
        ...
        invoice_account = self.env['account.account'].search(
            [('code', '=', '1122'), ('company_id', '=', self.env.user.company_id.id)], limit=1)
    ...
    def open_supplier_account_move_line_com(self):
        ...
        if sfk_type == 'yfhxd':
            account = self.env['account.account'].search(
                [('code', '=', account_type), ('company_id', '=', self.company_id.id)], limit=1)
    ...
    def _prepare_move_line_account(self, model=''):
        ...
        if model == 'payment':
            if payment_type == 'inbound':
                ...
                credit_account = account_obj.search(
                    [('code', '=', '1122'), ('company_id', '=', self.env.user.company_id.id)], limit=1)
                debit_sign, credit_sign = 1, -1
            else:
                debit_account = account_obj.search(
                    [('code', '=', '2202'), ('company_id', '=', self.env.user.company_id.id)], limit=1)
                credit_account = self.payment_account_id
                debit_sign, credit_sign = 1, -1
        elif model == 'advance':
            if payment_type == 'inbound':
                debit_account = account_obj.search(
                    [('code', '=', '2203'), ('company_id', '=', self.env.user.company_id.id)], limit=1)
                credit_account = account_obj.search(
                    [('code', '=', '1122'), ('company_id', '=', self.env.user.company_id.id)], limit=1)
                debit_sign, credit_sign = 1, -1
            else:
    ...
    def _prepare_sale_invoice_line(self, inv):  # 参考
        ...
        for line in inv.invoice_line_ids:
            if line.sale_line_ids or line.manual_so_id:
                so = line.sale_line_ids and line.sale_line_ids[0].order_id or line.manual_so_id
    ...
    def _prepare_purchase_invoice_line(self, inv):
        ...
        for line in inv.invoice_line_ids:
            if line.purchase_id or line.manual_po_id:
                po = line.purchase_id and line.purchase_id or line.manual_po_id
    ...
    def make_line_no(self):
        ...
        for one in self.invoice_ids:
            if amount > 0:
                ...
                if one.yjzy_type in ['sale', 'purchase'] or one.yjzy_type_1 in ['sale', 'purchase',
                                                                                'other_payment_sale',
                                                                                'other_payment_sale']:
                    ...
                    if one.declaration_amount >= one.residual:
                        amount_payment_org = one.residual
                        rest_amount += one.declaration_amount - one.residual
                    else:
                        amount_payment_org = one.declaration_amount
            ...
            print('amount_payment_org_akiny', amount_payment_org, rest_amount)
            if one.type in ['in_invoice', 'out_invoice', 'out_refund']:
            ...
            for one in self.line_no_ids:
                if self.sfk_type == 'yshxd':
                    ...
                    one.advice_amount_advance_org = line_id.so_tb_percent * one.yjzy_payment_id.amount + \
                                                    one.order_id.duoyu_this_time_advice_advance_org * (
                    ...
                    one.advice_amount_advance_org = line_id.so_tb_percent * one.yjzy_payment_id.amount + \
                                                    one.order_id.duoyu_this_time_advice_advance_org * (
                                                            amount_invoice_so / po_amount_all)
                    one.least_advice_amount_advance_org = least_advice_amount_advance_org
                    # one.advice_amount_advance_org = line_id.so_tb_percent * one.yjzy_payment_id.advance_balance_total
            ...
            if not po_invlines:
                ...
                for po, invlines in po_invlines.items():
                    if yjzy_advance_payment_id.po_id:
                        ...
                        yjzy_payment_id = yjzy_advance_payment_id.id
                    # 如果前端加了限制，这个就可以不用了
                    lines_same_count = len(self.line_ids.filtered(
                        lambda x: (x.yjzy_payment_id.id == yjzy_payment_id and x.invoice_id.id == invoice.id) or
...
class account_reconcile_order_line(models.Model):
    ...
    def compute_info(self):
        for one in self:
            ...
            one.amount_payment = payment_currency != False and payment_currency.compute(one.amount_payment_org,
                                                                                        company_currency)
    ...
    def _compute_amount_invoice_so_proportion(self):
        for one in self:
            amount_invoice = one.invoice_id.amount_total_signed
            amount_invoice_so = one.amount_invoice_so
        ...
        for one in self:
            if one.order_id.sfk_type == 'yfhxd':
                ...
                reconcile_line_done_ids = self.env['account.reconcile.order.line'].search(
                    [('order_id.state', 'in', ['done']), ('company_id', '=', one.company_id.id),
                     ('po_id', '=', one.po_id.id),
                     ('invoice_id', '=', one.invoice_id.id)])
                ...
                # 增加字段：认领后金额
                reconcile_line_ids = self.env['account.reconcile.order.line'].search(
                    [('order_id.state', 'in', ['post', 'done']), ('company_id', '=', one.company_id.id),
                     ('so_id', '=', one.so_id.id),
                     ('invoice_id', '=', one.invoice_id.id)])
                amount_payment_all = sum(x.amount_total_org_new for x in reconcile_line_ids)
                reconcile_line_done_ids = self.env['account.reconcile.order.line'].search(
    ...
    @api.depends('order_id.state', 'invoice_currency_id', 'payment_currency_id', 'currency_id', 'amount_advance_org',
                 'amount_payment_org')
    def compute_amount_total_org_new(self):
        for one in self:
            # date = one.order_id.date
    ...
    @api.depends('so_id', 'so_id.amount_total', 'so_id.no_sent_amount_new', 'amount_invoice_so', 'po_id.amount_total',
                 'po_id.no_deliver_amount_new', 'po_id')
    ...
    @api.depends('amount_invoice_so', 'amount_so', 'no_delivery_amount_so', 'order_id.yjzy_advance_payment_id.amount',
                 'order_id.yjzy_advance_payment_id', 'order_id')
    ...
    def compute_ysrld_amount_advance_org_all(self):
        for one in self:
            ...
            hxd_ids = one.yjzy_payment_id.advance_reconcile_order_line_ids.filtered(
                lambda x: x.invoice_id not in invoice_ids and x.amount_advance_org != 0 and x.order_id.state == 'done')
            hxd_self_ids = one.yjzy_payment_id.advance_reconcile_order_line_ids.filtered(lambda
                                                                                             x: x.invoice_id == one.invoice_id and x.amount_advance_org != 0 and x.order_id.state == 'done')  # 不等于0 是因为 老的数据，只要符合po相yjzy_payent同，就会给明细加上等于当前发票的核销单
            ...
            ysrld_advice_amount_advance_org_all = sum(
                x.advice_amount_advance_org_real for x in lines)  # ok # 之前是没有real的，计算错误，现在改成real
    ...
    def compute_amount_total_invoice(self):
        for one in self:
            one.amount_total_invoice = one.invoice_id.amount_total_signed
    ...
    def compute_amount_invoice_so_residual_can_approve_d_after(self):
        for x in self:
            ...
            amount_total_org_new = x.amount_total_org_new  # 需要加入币种计算  payment_currency.compute(one.amount_payment_org, invoice_currency)
            # amount_total_org_new = x.invoice_cyrrency_id.compute(x.amount_total_org_new,x.order_id.payment_currency_id)
    ...
    invoice_move_line_com_yfzk_ids = fields.One2many('account.move.line.com', '应付账单日志',
                                                     related='invoice_id.move_line_com_yfzk_ids')
    ...
    advice_amount_advance_org = fields.Monetary(u'建议预收金额', currency_field='yjzy_currency_id',
                                                compute=compute_so_tb_percent,
                                                store=True)  # 原则上的分配金额，就是销售采购占原始比例*原始的预收预付金额
    ...
    residual = fields.Monetary(related='invoice_id.residual_signed', string=u'发票余额', readonly=True,
                               currency_field='invoice_currency_id')
    ...
    no_delivery_amount_so = fields.Monetary(u'未发货销售或者采购金额', currency_field='invoice_currency_id',
                                            compute=compute_amount_so, store=True)
    ...
    # 826
    amount_invoice_so_residual = fields.Monetary(u'占比剩余应收付', currency_field='invoice_currency_id',
                                                 compute=compute_can_approve)  #
    ...
    amount_invoice_so_residual_d_after = fields.Monetary(u'本次认领后可认领金额', currency_field='invoice_currency_id',
                                                         compute=compute_amount_invoice_so_residual_can_approve_d_after)
    amount_invoice_so_residual_can_approve = fields.Monetary(u'占比剩余可申请的应收付', currency_field='invoice_currency_id',
                                                             compute=compute_can_approve)  #
    amount_invoice_so_residual_can_approve_d = fields.Monetary(u'静态占比剩余可申请的应收付', currency_field='invoice_currency_id')
    amount_invoice_so_residual_can_approve_d_after = fields.Monetary(u'本次认领后可申请金额',
                                                                     currency_field='invoice_currency_id',
                                                                     compute=compute_amount_invoice_so_residual_can_approve_d_after)
    advance_residual = fields.Monetary(currency_field='yjzy_currency_id', string=u'预付余额',
                                       compute=compute_advance_residual, store=True)
...
class account_reconcile_order_line_no(models.Model):
    ...
    def compute_amount_payment_can_approve_all_after(self):
        for line in self:
            line_ids_line = line.order_id.line_ids.filtered(
                lambda x: x.invoice_id == line.invoice_id)  # 本条发票未拆分明细对应的拆分明细
            # advance_amount_org = sum(x.amount_advance_org for x in line_ids_line)
            # advance_amount_org = line.amount_advance_org_compute #未考虑货币转换前
            amount_payment_can_approve_all_this_time = line.amount_payment_can_approve_all_this_time
            # amount_payment_org = line.amount_payment_org #未考虑货币转换前
    ...
    amount_total_org = fields.Monetary('总认领金额', currency_field='invoice_currency_id',
                                       compute=compute_amount_advance_org_compute, store=True)
    fkzl_id = fields.Many2one('account.payment', u'付款指令')
    ...
    invoice_declaration_amount_latest = fields.Monetary('本次退税申报金额', related='invoice_id.declaration_amount_latest',
                                                        store=True)
    ...
    invoice_residual = fields.Monetary(related='invoice_id.residual_signed', string=u'发票余额', readonly=True,
                                       currency_field='invoice_currency_id')
    residual = fields.Monetary(related='invoice_id.residual_signed', string=u'发票余额', readonly=True,
                               currency_field='invoice_currency_id')  # 有一个没用
    invoice_amount_total = fields.Monetary(related='invoice_id.amount_total_signed', string=u'发票金额', readonly=True,
                                           currency_field='invoice_currency_id')
    amount_payment_can_approve_all = fields.Monetary(related='invoice_id.amount_payment_can_approve_all',
                                                     string='可以申请支付应付款')
    amount_payment_can_approve_all_this_time = fields.Monetary(u'可以申请支付应付款d', currency_field='invoice_currency_id')

    invoice_residual_this_time = fields.Monetary(string=u'发票余额d', currency_field='invoice_currency_id')
    ...
    amount_advance_org_compute = fields.Monetary(u'计算的预收预付金额', currency_field='yjzy_currency_id',
                                                 compute=compute_amount_advance_org_compute, store=True)
    amount_advance_org = fields.Monetary(u'预收金额', currency_field='yjzy_currency_id')
    advice_amount_advance_org = fields.Monetary(u'建议预收金额', currency_field='yjzy_currency_id')
    least_advice_amount_advance_org = fields.Monetary(u'最低建议金额', currency_field='yjzy_currency_id')
...
class advance_payment_state(models.Model):
    ...
    amount_advance_balance = fields.Monetary(u'预收付款单余额', currency_field='advance_payment_currency',
    ...
    amount_advance_balance_after = fields.Monetary(u'本次认领后可认领金额', currency_field='advance_payment_currency',
                                                   compute=compute_amount_reconcile, store=True)
    amount_reconcile = fields.Monetary(u'本次认领金额', currency_field='advance_payment_currency',
                                       compute=compute_amount_reconcile, store=True)
