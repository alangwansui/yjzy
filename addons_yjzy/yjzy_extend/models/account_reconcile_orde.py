...
class account_reconcile_order(models.Model):
    ...
    invoice_amount_total = fields.Monetary('账单原始金额',currency_field='invoice_currency_id',related='invoice_id.amount_total_signed')
    ...
    def unlink(self):
        for one in self:
            if one.state_1 not in ['draft','draft_yshxd','draft_all','cancel']:
