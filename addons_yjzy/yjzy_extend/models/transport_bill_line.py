...
class transport_bill_line(models.Model):
    ...
    def get_purchase_cost(self):
        ...
        for i in self.lot_plan_ids.filtered(lambda x: x.stage_1):
            purchase_cost += i.lot_id.pol_id.currency_id.compute(i.qty * i.lot_id.purchase_price,
    ...
    @api.depends('lot_plan_ids', 'lot_plan_ids.qty', 'lot_plan_ids.lot_id.purchase_price', 'lot_plan_ids.stage_1',
                 'lot_plan_ids.lot_id.pol_id.currency_id')
    ...
    @api.depends('sol_id', 'lot_plan_ids', 'discount', 'lot_plan_ids.stage_2')
    def compute_org_currency_sale_amount(self):
        for one in self:
            sol = one.sol_id
            plan_lots = one.lot_plan_ids
            discount = one.discount
            ...
            org_currency_sale_amount = sol.price_unit * qty2stage * (1 - (discount or 0.0) / 100)
            org_currency_sale_amount_origin = sol.price_unit * qty2stage
            one.org_currency_sale_amount_new = org_currency_sale_amount
            one.org_currency_sale_amount_origin_new = org_currency_sale_amount_origin
    ...
    company_currency_id = fields.Many2one('res.currency', string='公司货币', related='bill_id.company_currency_id',
                                          readonly=True)
    ...
    org_currency_sale_amount = fields.Monetary(u'销售货币金额', currency_field='sale_currency_id', compute=compute_info,
                                               store=False, )
    org_currency_sale_amount_new = fields.Monetary(u'销售货币金额', currency_field='sale_currency_id',
                                                   compute=compute_org_currency_sale_amount, store=True)
    org_currency_sale_amount_discount = fields.Monetary(u'折扣', currency_field='sale_currency_id',
                                                        compute=compute_info, store=False)
    org_currency_sale_amount_origin = fields.Monetary(u'销售货币金额', currency_field='sale_currency_id',
                                                      compute=compute_info, store=False)
    org_currency_sale_amount_origin_new = fields.Monetary(u'销售货币金额', currency_field='sale_currency_id',
                                                          compute=compute_org_currency_sale_amount, store=True)
    ...
    purchase_cost = fields.Monetary(u'采购成本', currency_field='company_currency_id', compute=compute_info,
                                    digits=dp.get_precision('Money'))
    # 1102
    purchase_cost_new = fields.Monetary(u'采购成本', currency_field='company_currency_id',
    ...
    def _prepare_2_picking(self):
        ...
        pick_type = self.env['stock.picking.type'].search(
            [('ref', '=', 'delivery'), ('company_id', '=', self.env.user.company_id.id)], limit=1)
