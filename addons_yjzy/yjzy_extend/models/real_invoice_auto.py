...
class PlanInvoiceAuto(models.Model):
    ...
    def compute_plan_invoice_auto_amount(self):
        for one in self:
            one.plan_invoice_auto_amount = sum(x.amount_total for x in one.invoice_ids)
        ...
        for one in self:
            ...
            if not date_ship:
                ...
                if one.state_1 == '20':
                    ...
                    print('invoice_akiny',one.state_1, len(back_tax_invoice_declare_ids), len(back_tax_invoice_ids),back_tax_invoice_declare_ids,back_tax_invoice_ids)
