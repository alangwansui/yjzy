...
class PoRefuseWizard(models.TransientModel):
    ...
    _description = "Purchase Order Refuse Reason Wizard"
    ...
    def default_get(self, fields):
        res = super(PoRefuseWizard, self).default_get(fields)
        active_ids = self.env.context.get('active_ids', [])
        res.update({
            'po_id': active_ids[0] if active_ids else False,
            })
        return res
    #写上拒绝的理由，sale_order上执行拒绝
    def po_refuse_reason(self):
        self.ensure_one()
        self.po_id.action_refuse_stage(self.reason)
